...
public class GUIListagemCliente extends JDialog {
    ...
    public GUIListagemCliente(List<Cliente> texto) {
        ...
        String[] colunas = new String[]{"CPF", "Nome", "Data de nascimento", "Sexo", "Contato",};
        String[][] dados = new String[0][3];
        DefaultTableModel model = new DefaultTableModel(dados, colunas);
        JTable tabela = new JTable(model);
        scroll.setViewportView(tabela);
        for (int i = 0; i < texto.size(); i++) {
            String[] linha = new String[]{String.valueOf(texto.get(i).getCpf()), String.valueOf(texto.get(i).getNome()), sdf.format(texto.get(i).getDataNascimento()), String.valueOf(texto.get(i).getIdSexo()), String.valueOf(texto.get(i).getContato()),};
            model.addRow(linha);
        }
        painelTa.add(scroll);
