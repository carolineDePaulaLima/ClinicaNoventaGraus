...
public class GUIEspecialidades extends JDialog {
    ...
    private EspecialidadesTableModel tableModel;

    private DAOEspecialidades daoEspecialidades = new DAOEspecialidades();

    ;

    public GUIEspecialidades() {

        setTitle("CRUD - Especialidades");
        ...
        tableModel = new EspecialidadesTableModel(lista);
...
//        painelBotoes.add(btnRem);
            ...
            public void actionPerformed(ActionEvent evt) {
                if (table.getSelectedRow() >= 0) {
                    ...
                    if (JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog(cp,
                            "Confirme a exclusão [" + tableModel.getValue(table.getSelectedRow()).getIdEspecialidades() + " - "
                            + tableModel.getValue(table.getSelectedRow()).getNomeEspecialidade()+ "]?", "Confirm",
        ...
        btnAdd.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                especialidades.setIdEspecialidades(daoEspecialidades.autoIdEspecialidades());
                especialidades.setNomeEspecialidade("Nome");

                try {
                    Especialidades c = daoEspecialidades.obter(especialidades.getIdEspecialidades());
                    if (c == null) {
                        daoEspecialidades.inserir(especialidades);
                        tableModel.onAdd(especialidades);
                        tableModel.fireTableDataChanged();
                    } else {
                        JOptionPane.showMessageDialog(cp, "Já existe");
                    }
                } catch (Exception err) {
                    JOptionPane.showMessageDialog(cp, "Erro ao inserir =>" + especialidades.getIdEspecialidades()
        ...
        btnRem.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (table.getSelectedRow() != -1 && table.getSelectedRow() < tableModel.getRowCount()) {
                    Especialidades c = tableModel.getValue(table.getSelectedRow());
                    daoEspecialidades.remover(c);
                    tableModel.onRemove(table.getSelectedRows());

                } else {
                    JOptionPane.showMessageDialog(cp, "Escolha na tabela o especialidades a ser excluído");
                    table.requestFocus();
                }
                tableModel.fireTableDataChanged();
            }
        ...
        btnCarregar.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    List<Especialidades> lc = daoEspecialidades.list();
