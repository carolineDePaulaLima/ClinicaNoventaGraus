...
public class GUITipoContato extends JDialog {
    ...
    private TipoContatoTableModel tableModel;

    private DAOTipoContato daoTipoContato = new DAOTipoContato();

    ;

    public GUITipoContato() {

        setTitle("CRUD - TipoContato");
        ...
        tableModel = new TipoContatoTableModel(lista);
...
//        painelBotoes.add(btnRem);
            ...
            public void actionPerformed(ActionEvent evt) {
                if (table.getSelectedRow() >= 0) {
                    ...
                    if (JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog(cp,
                            "Confirme a exclusão [" + tableModel.getValue(table.getSelectedRow()).getIdTipoContato() + " - "
                            + tableModel.getValue(table.getSelectedRow()).getNomeContato()+ "]?", "Confirm",
        ...
        btnAdd.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                tipoContato.setIdTipoContato(daoTipoContato.autoIdTipoContato());
                tipoContato.setNomeContato("Nome");

                try {
                    TipoContato c = daoTipoContato.obter(tipoContato.getIdTipoContato());
                    if (c == null) {
                        daoTipoContato.inserir(tipoContato);
                        tableModel.onAdd(tipoContato);
                        tableModel.fireTableDataChanged();
                    } else {
                        JOptionPane.showMessageDialog(cp, "Já existe");
                    }
                } catch (Exception err) {
                    JOptionPane.showMessageDialog(cp, "Erro ao inserir =>" + tipoContato.getIdTipoContato()
        ...
        btnRem.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (table.getSelectedRow() != -1 && table.getSelectedRow() < tableModel.getRowCount()) {
                    TipoContato c = tableModel.getValue(table.getSelectedRow());
                    daoTipoContato.remover(c);
                    tableModel.onRemove(table.getSelectedRows());

                } else {
                    JOptionPane.showMessageDialog(cp, "Escolha na tabela o tipoContato a ser excluído");
                    table.requestFocus();
                }
                tableModel.fireTableDataChanged();
            }
        ...
        btnCarregar.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    List<TipoContato> lc = daoTipoContato.list();
    ...
    private static class DateEditor extends AbstractCellEditor implements TableCellEditor {
        ...
        public static void main(String[] args) {
            GUITipoContato guiTipoContato = new GUITipoContato();
        }
    }
}
